...
var Autocomplete = React.memo(function (props) {
  ...
  var optionStyles = Object.assign({}, _objectSpread({}, props.height === heights.SHORT ? autocompleteShortStyles : null, {
  ...
  return equals(cp.isValid, np.isValid) && equals(cp.options, np.options) && equals(cp.payload, np.payload) && equals(cp.value, np.value) && equals(cp.height, np.height) && equals(cp.width, np.width) && equals(cp.short, np.short);
});
